...
<html lang="en">
<head>
<title> All Featured Jobs |  <?php echo $title; ?> </title>
...
<?php include 'filter.php';?>
      ...
      <div class="col-md-9 col-sm-8 jobs-list-all">
		...
		if($c = $_SESSION["c"]){
				$c = $_SESSION["c"];
				$user_resume = mysqli_query($con, "SELECT * FROM job_listing WHERE fea_job = '1' and status = '1' and country = '$c' order by id desc");
			} else {
		
			$user_resume = mysqli_query($con, "SELECT * FROM job_listing WHERE fea_job = '1' and status = '1' order by id desc");
...
<script type="text/javascript" src="http://hassandesigns.top/html/jobfinder/rs-plugin/js/jquery.themepunch.revolution.min.js"></script>

<script type="text/javascript" src="http://hassandesigns.top/html/jobfinder/rs-plugin/js/jquery.themepunch.tools.min.js"></script>
